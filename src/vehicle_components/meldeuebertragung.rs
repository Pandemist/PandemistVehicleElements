...
use crate::{mocks::communication::send_message_to_trigger, structs::internal_events::InternalEvents};
...
impl Meldeuebertragung {
	...
	pub fn update_sensor_value(&mut self, sensor_id: i32, sensor_activ: bool, id: String, sensor_val: String) {
		self.sensor_map.insert(sensor_id, MueEntry{aktiv: sensor_activ, id: id, value: sensor_val});
	}
	...
	pub fn trigger_handler(&mut self, id: &str, sensor_index: i32) {
		if !self.iws_defect {
			...
			match map_entry {
				Some(v) => {
					if v.aktiv {
						handle_reply(send_message_to_trigger(v.id.clone(), v.value.clone(), sensor_index));
					}
				},
				None => {},
			}
