...
use crate::{
    elements::tech::{
        blinker::Blinkrelais,
        slider::{AntiGravitySlider, InertionSlider},
    },
    mocks::key_event::KeyEventCab,
};
...
impl Klapprampe {
    pub fn new(name: &str, cab_side: KeyEventCab) -> Self {
        Klapprampe {
            ...
            abdeckung: AntiGravitySlider::new(
                &format!("{}_abdeckung", name),
                cab_side,
                0.5,
                2.4,
                1.0 / 250.0,
            ),
            rampe_bahnsteig: InertionSlider::new(
                &format!("{}_rampe_bahnsteig", name),
                cab_side,
                10.0,
                -1.0,
                0.25,
            ...
            warnrelais: Blinkrelais::new(
                KLAPPRAMPE_DOORWARN_INTERVAL,
                KLAPPRAMPE_DOORWARN_INTERVAL_HALF,
                0.1,
            ),
            lockpin: false,
            slider_pos: 0.0,
            slider_speed: 0.0,
            slider_new_speed: 0.0,
            slider_grabbing: false,
            slider_daempfer: 0.0,
            slider_bumpfactor: 0.0,
            can_open: false,
            can_close: true,
        }
    }
    ...
    pub fn tick(&mut self, mouse_x: f32, mouse_y: f32, door_open_lock: bool) {
        ...
        self.abdeckung.max = abdeckung_upper_bump;
        if self.lockpin {
            self.abdeckung.tick();
        }
        ...
        if self.abdeckung.pos > 0.3333 {
            ...
            if self.slider_speed != 0.0 {
                self.slider_new_speed = self.slider_speed
                    + (-self.slider_speed.signum() * self.slider_daempfer) * delta();
        ...
        self.rampe_bahnsteig
            .tick((-mouse_y / 5.0) * (self.lockpin as i32 as f32));
    }
}
