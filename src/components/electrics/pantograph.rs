...
/// A third rail collector for collecting power from third rail systems.
///
/// This component simulates a realistic third rail power collection system
/// commonly used in subway and urban rail networks. It includes automatic
/// deployment, sparking effects, and safety interlocks.
///
/// # Features
///
/// - Automatic deployment with configurable speeds
/// - Realistic sparking effects on connection/disconnection
/// - Sound effects for different operational states
/// - Safety interlocks requiring battery and safeguard systems
/// - Side-specific operation (left/right rail)
/// - State-dependent voltage output
///
/// # Safety
///
/// The collector includes multiple safety features:
/// - Requires battery power to operate
/// - Requires safeguard systems to be active
/// - Automatic shutdown in unsafe conditions
/// - Proper sequencing of connection/disconnection
#[derive(Debug)]
...
impl ThirdRailCollector {
    /// Creates a new third rail collector.
    ///
    /// # Arguments
    ///
    /// * `anim_name` - Name for the collector animation
    /// * `snd_on_name` - Optional sound for activation
    /// * `snd_off_name` - Optional sound for deactivation
    /// * `snd_anlauf_name` - Optional sound for initial contact
    /// * `snd_ablauf_name` - Optional sound for contact loss
    /// * `lm_spark_name` - Optional light for sparking effects
    /// * `id` - Unique identifier for this collector
    /// * `side` - Which side of the track (left/right)
    /// * `spark_time` - Base duration for spark effects (seconds)
    /// * `spark_variance` - Random variance in spark duration (seconds)
    /// * `spark_on_connect` - Whether to spark when connecting
    /// * `spark_on_disconnect` - Whether to spark when disconnecting
    /// * `move_up` - Speed for raising the collector
    /// * `move_down` - Speed for lowering the collector
    ///
    /// # Returns
    ///
    /// A new `ThirdRailCollector` instance configured according to the
    /// global vehicle initialization state.
    ///
    /// # Example
    ///
    /// ```rust
    /// let collector = ThirdRailCollector::new(
    ///     "collector_anim",
    ///     Some("motor_on"),
    ///     Some("motor_off"),
    ///     Some("rail_contact"),
    ...
    ///     0.5,  // spark time
    ///     0.1,  // spark variance
    ///     true, // spark on connect
    ///     true, // spark on disconnect
    ///     2.0,  // move up speed
    ///     1.5,  // move down speed
    /// );
    /// ```
    ...
    pub fn new(
        ...
        snd_on_name: Option<&str>,
        snd_off_name: Option<&str>,
        snd_anlauf_name: Option<&str>,
        snd_ablauf_name: Option<&str>,
        lm_spark_name: Option<&str>,
        id: usize,
        side: Side,
        spark_time: f32,
        spark_variance: f32,
        ...
        let mut panto = ThirdRailCollector {
            ...
            anim: Animation::new(Some(&anim_name.into())),
            voltage_norm: 0.0,
            ...
            snd_on: Sound::new_simple(snd_on_name),
            snd_off: Sound::new_simple(snd_off_name),
            snd_anlauf: Sound::new_simple(snd_anlauf_name),
            snd_ablauf: Sound::new_simple(snd_ablauf_name),
            spark: Light::new(lm_spark_name),
        };
        ...
        match init_ready_state() {
            ...
            VehicleInitState::Setuped | VehicleInitState::ReadyToDrive => {
                panto.state = SwitchingState::On;
                panto.pos = 1.0;
            }
        }
        panto.anim.set(panto.pos);

        panto
    }

    /// Updates the third rail collector state for one simulation tick.
    ///
    /// This method handles all collector logic including:
    /// - Motor control and movement based on target state
    /// - Safety system integration (battery and safeguard)
    /// - Third rail contact state monitoring
    /// - Spark effect generation during state transitions
    /// - Sound effect management
    /// - Voltage calculation based on contact and power usage
    ///
    /// # Arguments
    ///
    /// * `target` - Desired switching target (On/Off/Neutral with optional delay)
    /// * `power_usage` - Whether the vehicle is currently using power
    /// * `batterie` - Whether battery power is available
    /// * `safeguard` - Whether safety systems are active
    ///
    /// # Safety
    ///
    /// The collector will not operate if either `batterie` or `safeguard` is false.
    /// This prevents operation during unsafe conditions or power failures.
    ///
    /// # Sparking Behavior
    ///
    /// Sparks are generated during rail contact transitions when:
    /// - `power_usage` is true
    /// - `spark_on_connect` or `spark_on_disconnect` is enabled
    /// - The collector is in the raised position
    /// - A state transition occurs between Connected and PartwiseConnected
    ///
    /// # Example
    ///
    /// ```rust
    /// // Raise collector with 1 second delay, with power usage active
    /// collector.tick(
    ///     SwitchingTarget::TurnOn(1.0),
    ///     true,  // power usage active
    ///     true,  // battery available
    ///     true,  // safeguard active
    /// );
    /// ```
