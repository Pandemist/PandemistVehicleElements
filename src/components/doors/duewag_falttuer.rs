...
use super::door_enums::{DoorState, DoorTarget};
...
pub struct DueWagFalttuer {
    ...
    key_grab_aa: KeyEvent,
    key_grab_ab: KeyEvent,
    key_grab_ba: KeyEvent,
    key_grab_bb: KeyEvent,
...
impl DueWagFalttuer {
    pub fn new(name: String, door_index_1: usize, door_index_2: usize) -> Self {
        ...
        let mut tuer = DueWagFalttuer {
            ...
            key_grab_aa: KeyEvent::new(format!("{}_grab_aa", name), KeyEventCab::Indifferent),
            key_grab_ab: KeyEvent::new(format!("{}_grab_ab", name), KeyEventCab::Indifferent),
            key_grab_ba: KeyEvent::new(format!("{}_grab_ba", name), KeyEventCab::Indifferent),
            key_grab_bb: KeyEvent::new(format!("{}_grab_bb", name), KeyEventCab::Indifferent),
            ...
            motor_rot_last: 0.0,
            motor_pos: 0.0,
            motor_correct_delay: 0.0,
            motor_move: 0,
            lichtschranke_aktiv: false,
            motor_endlage_auf: false,
            closing_timer: 0.0,
            fluegel_a_endlage_auf: false,
            fluegel_b_endlage_auf: false,
        };
    ...
    pub fn handmove(&mut self) {
        let grabbing_aa = self.key_grab_aa.is_pressed();
        let grabbing_ab = self.key_grab_ab.is_pressed();
        let grabbing_ba = self.key_grab_ba.is_pressed();
        let grabbing_bb = self.key_grab_bb.is_pressed();

        let move_delta = mouse_move().x / 500.0;
        if grabbing_aa || grabbing_ab {
            if grabbing_aa {
    ...
    pub fn tick(
        ...
        self.wing_a_anim.set(self.wing_a_pos);
        self.wing_b_anim.set(self.wing_b_pos);
