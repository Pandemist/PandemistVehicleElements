...
use crate::{
    elements::{std::helper::gen_f32_range, tech::blinker::Blinkrelais},
    mocks::{
        animation::Animation, generell::mouse_move, sound::Sound, sound::SoundTarget,
...
impl AegElectricDoor {
    pub fn new(name: String, door_index: usize, serie_1: bool) -> Self {
        let mut door = AegElectricDoor {
            name_id: name.clone(),
            plug_radius: 0.06,
            shift: 0.58,
            friction: 0.05,
            open_start_speed: gen_f32_range(0.58..=0.65),
            open_end_speed: 0.3,
            open_start_end_change_pos: 0.6,
            close_start_speed: gen_f32_range(0.45..=0.5),
            close_end_speed: 0.1,
            ...
            snd_door_warn: Sound::new(format!("snd_{}_warn", name)),
            ...
            kiwa_rolli: false,
            lm_warn_in: 0.0,
            notentriegelung: false,
            notentriegelung_last: false,
            door_mode_last: DoorTarget::Zu,
            door_1_last: false,
            new_mode: DoorTarget::Zu,
            auf_flag: false,
            closed_while_warning: false,
        };

        if serie_1 {
            door.reflection_open = gen_f32_range(0.03..=0.05);
            door.reflection_close = 0.05;
        } else {
            door.reflection_open = gen_f32_range(0.05..=0.07);
    ...
    fn move_door(&mut self, a: f32) {
        ...
        self.anim_x.set(self.pos_x);
        self.anim_y.set(self.pos_y);
    }
    ...
    pub fn tick(
        ...
        let mouse_delta_x = mouse_move().x * 0.1;

        if self.notentriegelung || !(self.energy && self.sicherung && self.pos > 0.01) {
            if self.grabbing_a {
                self.pos = (self.pos - mouse_delta_x * delta()).min(1.0).max(0.0);
