...
use crate::{
    ...
    structs::{
        internal_enums::{
            SoundTarget, SwitchingState, SwitchingTarget, ThirdRailSide, ThirdRailState,
            VehicleInitState,
        },
        traits::ElectricEquipment,
    },
};
...
impl Pantograph {
    pub fn new(
        ...
        let mut panto = Pantograph {
            ...
            api_panto: ApiPantograph::new(name.clone()),
    ...
    pub fn tick(
        ...
        self.anim.update_pos(self.panto_pos);
    }
}

impl ElectricEquipment for Pantograph {
    fn voltage(&self) -> f32 {
        self.voltage_norm
    }
}
...
impl ThirdRailCollector {
    pub fn new(
        ...
        let mut panto = ThirdRailCollector {
            ...
            api_thirdrailcollector: ApiThirdRailCollector::new(name.clone(), side),

            snd_on: Sound::new(format!("snd_{}_on", name)),
            snd_off: Sound::new(format!("snd_{}_off", name)),
            snd_anlauf: Sound::new(format!("snd_{}_anlauf", name)),
            snd_ablauf: Sound::new(format!("snd_{}_ablauf", name)),

            spark: Light::new(format!("light_{}", name)),
        };
    ...
    pub fn tick(
            ...
            && (self.third_rail_state_last == ThirdRailState::Connected)
        {
            ...
            if self.spark_on_connect && power_usage {
                ...
                self.spark_timer = rng.gen_range(
...
impl ElectricEquipment for ThirdRailCollector {
    fn voltage(&self) -> f32 {
        self.voltage_norm
    }
}
